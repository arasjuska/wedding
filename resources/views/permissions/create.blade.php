<x-back-layout>
    <x-slot name="header">
        ...
        <x-link href="{{ route('dashboard.admin.permissions.index') }}">
            {{ __('Back') }}
        </x-link>
    </x-slot>
                        ...
                        <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
                            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                                <div class="text-gray-600">
                                    <p class="font-medium text-lg">{{ __('Add new permission to user') }}</p>
                                    <p>{{ __('Please fill out all the fields.') }}</p>
                                </div>

                                <form action="{{ route('dashboard.admin.permissions.store') }}" method="post" class="lg:col-span-2">
                                    @csrf
